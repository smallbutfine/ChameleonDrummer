...
begin
  ...
  JsonValue := TJSONObject.ParseJSONValue(FileToStr(ASidecarPath));
  try
...
begin
  ...
  JsonValue := TJSONObject.ParseJSONValue(FileToStr(ASongMapPath));
  try
    if not Assigned(JsonValue) or (JsonValue.JsonType <> jtObject) then Exit;
    var Root := JsonValue.AsObject;

    Title := Root.Values['title'].AsString;
    var RegionsVal := Root.Values['regions'];
    if not Assigned(RegionsVal) or (RegionsVal.JsonType <> jtArray) then Exit;
    RegionsArr := RegionsVal.AsArray;
